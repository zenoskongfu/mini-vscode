...
function createLanguageServiceManager(workspace) {
	...
	const openDoc = (fileName) => workspace.textDocuments.find((d) => d.uri.path === fileName) || null;
	...
	function ensureService(rootDir) {
		...
		const host = {
			...
			getScriptFileNames: () =>
				workspace.textDocuments.filter((d) => /\.(ts|tsx|js|jsx)$/.test(d.uri.path)).map((d) => d.uri.path),
			getScriptVersion: (f) => {
				const d = openDoc(f);
